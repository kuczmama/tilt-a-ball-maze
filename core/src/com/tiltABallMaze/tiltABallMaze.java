...
public class tiltABallMaze extends ApplicationAdapter {
	...
	private void calculateImageLocation(){	
		...
		/*if(desktop){
			...
			int value = level1.getPixel((int)ballX,(int)ballY);
			R = ((value & 0xff000000) >>> 24);
			G = ((value & 0x00ff0000) >>> 16);
			B = ((value & 0x0000ff00) >>> 8);
			A = ((value & 0x000000ff));
			...
			//if(isWhite(level1.getPixel((int)(ballX + deltaY),(int)(ballX + deltaX)))){
				ballX += deltaX;
				ballY += deltaY;
			//}
			//if(!isWhite(level1.getPixel((int)(ballX + deltaY),(int)(ballX + deltaX)))) {
	...
	private void convertPixmap(Pixmap pixmap){
	...
	public void render () {
		...
		batch.begin();
		batch.draw(level1tex,screenWidth,screenHeight);
		//batch.draw(level1tex,0,0);
		//batch.draw(level1tex,0,0,screenWidth,screenHeight);
		//level1.drawPixmap(pixmap, 0, 0, (int)screenWidth, (int)screenHeight, A, A);
		batch.draw(img, ballX, ballY,boxSize,boxSize);
		
		font.draw(batch, R + " " + B + " " + G + " " + A + " " + (int)ballX + " " + (int)ballY + " " + level1.getWidth() + " " + screenWidth, 50, 50);
